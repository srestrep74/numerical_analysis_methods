...
def newton_multiple_roots_method(initial_guess, tolerance, max_iterations, function_expression, derivative_expression, second_derivative_expression):
    ...
    while error > tolerance and function_current != 0 and derivative_current != 0 and iteration_count < max_iterations:
        # Update the next x_n+1 based on the multiple roots formula
        denominator = (derivative_current ** 2) - (function_current * second_derivative_current)
        if denominator == 0:
            print("Denominator became zero, stopping iterations.")
            break
        x_next = x_current - (function_current * derivative_current) / denominator
        
        # Update derivative and function values
        ...
        second_derivative_current = evaluate_function(second_derivative_expression, x_next)
...
def main():
    ...
    second_derivative_expression = input("Enter the second derivative function f''(x) (use 'x' as the variable): ")

    # Run the Newton method for multiple roots
    root_approximations, function_values, errors, iteration_numbers = newton_multiple_roots_method(
        initial_guess, tolerance, max_iterations, function_expression, derivative_expression, second_derivative_expression)
